...
export const BuyerDashboard: React.FC<BuyerDashboardProps> = ({ user, onUpdateUser }) => {
  ...
  const OrderStatusStepper = ({ status }: { status: string }) => {
    ...
    return (
      <div className="flex items-center w-full max-sm:w-full gap-2">
        {steps.map((s, i) => (
  ...
  if (step === 'tracking') {
    return (
      <div className="max-w-5xl mx-auto px-4 py-20 animate-in fade-in duration-500 text-left">
        ...
        <div className="flex justify-between items-end mb-12">
            <div>
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter">Order History</h1>
                <p className="text-slate-500 font-medium italic mt-2">Tracking your industrial fuel allocations.</p>
            </div>
            <div className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center">
                <ShoppingBag className="mr-3 h-4 w-4 text-green-500" /> Supplies: {orders.length}
            </div>
        </div>
        ...
        {orders.length === 0 ? (
          <div className="bg-slate-50 p-20 rounded-[4rem] text-center border border-slate-100">
             ...
             <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No active supply records.</p>
          </div>
        ) : (
  ...
  if (step === 'selection') {
    return (
      <div className="max-w-4xl mx-auto px-4 py-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <p className="text-slate-500 text-center mb-16 font-medium text-lg italic text-center">Select resource for procurement.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <button onClick={() => handleSelection('biodiesel')} className="group bg-white p-12 rounded-[3.5rem] border-2 border-slate-100 hover:border-green-500 hover:shadow-2xl transition-all text-left relative overflow-hidden">
            ...
            <p className="text-slate-500 font-medium leading-relaxed italic">Powering engines with {DIESEL_PRICE_PER_LITER} BDT/L bio-fuel.</p>
            <div className="mt-8 flex items-center text-green-600 font-black uppercase tracking-widest text-xs">
              Configure Supply <ArrowRight className="ml-2 h-4 w-4" />
            </div>
          </button>
          ...
          <button onClick={() => handleSelection('glycerin')} className="group bg-white p-12 rounded-[3.5rem] border-2 border-slate-100 hover:border-emerald-500 hover:shadow-2xl transition-all text-left relative overflow-hidden">
            ...
            <p className="text-slate-500 font-medium leading-relaxed italic">Industrial byproduct for chemical manufacturing.</p>
            <div className="mt-8 flex items-center text-emerald-600 font-black uppercase tracking-widest text-xs">
              Bulk Inquiry <ArrowRight className="ml-2 h-4 w-4" />
            </div>
          </button>
        </div>
        <div className="mt-12 text-center">
            <button onClick={() => setStep('tracking')} className="text-slate-400 font-black uppercase tracking-widest text-[10px] hover:text-green-600 transition-colors flex items-center justify-center mx-auto">
                <History className="mr-2 h-3 w-3" /> View Order Tracker
            </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-8 text-left">
        <div>
          ...
          <div className="flex gap-4 mt-2">
            ...
            <button onClick={() => setStep('tracking')} className="text-slate-500 uppercase text-[10px] font-black tracking-widest flex items-center hover:text-green-600 transition-colors">
              <History className="h-3 w-3 mr-2" /> View Tracker
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-left">
            ...
            <div className="bg-white rounded-[4rem] shadow-sm border border-slate-100 p-12">
               ...
               <div className="p-10 bg-slate-900 rounded-[3rem] text-white flex flex-col md:flex-row justify-between items-center mb-12 relative overflow-hidden group gap-8">
                  <div className="relative z-10">
                    <p className="text-[10px] text-green-400 font-black uppercase tracking-widest mb-2">Total Payable (@ {DIESEL_PRICE_PER_LITER} ৳)</p>
                    <p className="text-5xl font-black tracking-tight">{totalCost.toLocaleString()} ৳</p>
                  </div>
                  <Button size="lg" className="rounded-2xl h-16 px-12 shadow-2xl relative z-10 w-full md:w-auto font-black bg-green-600" onClick={handlePurchaseAttempt}>
        ...
        <div className="space-y-8">
           <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100">
              ...
              <p className="text-sm text-slate-600 font-medium italic leading-relaxed">Verification of industrial trade license is required for high-volume orders. Track progress in real-time via the Tracker.</p>
           </div>
        </div>
      </div>
      ...
      {isInsufficientModalOpen && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
           <div className="bg-white rounded-[3rem] w-full max-w-lg p-12 relative shadow-2xl border-4 border-red-500/20 text-left">
              ...
              <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tighter uppercase leading-none">Low Balance</h2>
              <p className="text-slate-500 font-medium italic mb-10 leading-relaxed">Your account balance is too low. Deposit funds or use an authorized referral code for cash payment.</p>
              ...
              <div className="space-y-6">
                 <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center">
                       <Ticket className="h-3 w-3 mr-2 text-green-600" /> Pay as Cash (Authorized Code)
                    </label>
                    ...
                    {referralError && <p className="text-[10px] font-black text-red-600 uppercase mt-3 text-center tracking-widest">Invalid or Expired Code</p>}
                 </div>
                 ...
                 >
                   <Plus className="mr-3 h-5 w-5" /> Deposit Now
                 </Button>
              </div>
           </div>
        </div>
      ...
      {isConfirmModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
           <div className="bg-white rounded-[3rem] w-full max-w-lg p-12 relative shadow-2xl border border-slate-100 text-left">
              ...
              <h2 className="text-4xl font-black text-slate-900 mb-8 tracking-tighter">Confirm Supply</h2>
              <div className="space-y-6 mb-10 bg-slate-50 p-8 rounded-3xl">
                 ...
                 <div className="flex justify-between py-4 border-b border-slate-200">
                    <span className="text-slate-500 font-bold uppercase text-xs">Volume</span>
                    <span className="text-slate-900 font-black">{liters} L</span>
                 </div>
                 <div className="flex justify-between py-4 text-2xl pt-6">
                    <span className="text-slate-900 font-black uppercase tracking-tighter text-3xl">Total Cost</span>
                    <span className="text-green-600 font-black text-3xl">{totalCost.toLocaleString()} ৳</span>
                 </div>
              </div>
              <div className="flex gap-4">
                 <Button variant="outline" className="flex-1 h-16 rounded-2xl font-black" onClick={() => setIsConfirmModalOpen(false)}>Cancel</Button>
                 <Button className="flex-[2] h-16 rounded-2xl font-black bg-green-600" onClick={() => finalizeOrder('wallet')}>Confirm Order</Button>
              </div>
           </div>
        </div>
      )}
      ...
      {/* Recharge Modal */}
      {isRechargeModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xl">
           <div className="bg-white rounded-[3rem] w-full max-w-md p-10 relative shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
              ...
              {rechargeStep === 'method' ? (
                <div className="space-y-8 text-left">
                   ...
                   <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Amount (৳)</label>
                ...
                <div className="space-y-8 animate-in slide-in-from-right-4 duration-300 text-left">
                   <div className={`p-8 rounded-[2rem] text-white ${selectedGateway === 'bkash' ? 'bg-[#E2136E]' : 'bg-[#ED1C24]'}`}>
                      <p className="text-[10px] font-black uppercase opacity-80 mb-1">Send to Merchant No.</p>
                      <p className="text-2xl font-black">01799624541</p>
                   </div>
                   <div>
                      <input type="text" required value={txnId} onChange={(e) => setTxnId(e.target.value)} className="w-full px-6 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-slate-900 outline-none text-xl font-black text-center uppercase tracking-widest" placeholder="TXN ID" />
                      <div className="flex gap-4 mt-8">
                         <Button variant="outline" className="flex-1 h-16 rounded-2xl" onClick={() => setRechargeStep('method')}>Back</Button>
                         <Button className="flex-[2] h-16 rounded-2xl font-black bg-slate-900" onClick={finalizeRecharge}>Submit</Button>
                      </div>
                   </div>
                </div>
              )}
